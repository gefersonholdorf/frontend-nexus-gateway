...
`src/main.tsx` já referencia `src/modules/core/pages/{core-home-page, core-users-page, core-roles-page, core-permissions-page, core-modules-page, core-module-detail-page, core-integrations-page, core-audit-page}.tsx` e `src/components/menu/sidebar-module.tsx` já aponta os itens de "Administração" para `/core/*`. **Nenhum desses arquivos existe no disco.** A implementação anterior (`src/modules/{users,rbac,modules,integrations,providers}`, do commit `863b840`) foi deletada para dar lugar a essa reconstrução única sob `src/modules/core`. `PermissionProvider` também foi removido junto (ver [auth-and-rbac.md](auth-and-rbac.md)) — o build não compila até pelo menos a Etapa 1 (fundação) ser refeita.

A spec abaixo é a última definida (recuperada de `docs/features/core/*` do commit `863b840`) e reflete a decisão de **reconstruir o módulo do zero, 100% mockado via TanStack Query, sem chamadas HTTP reais**, antes de plugar no backend V2. Trate como fonte de verdade para "o que é o módulo Core", mesmo que os arquivos originais tenham sido removidos.

## Restrição transversal

Todas as etapas usam dados mockados (arrays em memória ou fixtures), consumidos como se fossem resposta de API através de `useQuery`/`useMutation` do TanStack Query. Nenhuma etapa faz `fetch` real nem usa `ApiClient`/V2 ainda — isso é proposital, para permitir construir a UI antes do contrato de API estar fechado. Não introduza chamadas HTTP reais em nenhuma etapa a menos que o usuário peça explicitamente essa mudança de escopo.
